...
public class ProgressEvaluator : MonoBehaviour
{
    ...
    private void PassBand()
    {
        ...
        if (!VectApprox(redSphere.transform.position, new Vector3(2.5f, 0, 5)))
            throw new EvalFailedException("RedSphere is not located at position (2.5, 0, 5).");
    ...
    private void CreditBand()
    {
        ...
        if (!VectApprox(woodSphere.transform.position, new Vector3(-2.5f, 0, 5)))
            throw new EvalFailedException("WoodSphere is not located at position (-2.5f, 0, 5).");
    ...
    private void DistinctionBand()
    {
        ...
        if (!VectApprox(spotLight.transform.position, new Vector3(3.5f, -3, 3)))
    ...
    private void HighDist90Band()
    {
        if (!VectApprox(Camera.main.transform.position, new Vector3(0, 1, 1)) &&
            !VectApprox(Camera.main.transform.position, new Vector3(0, 1, 1.5f)))
        ...
        if (RenderSettings.skybox != null)
            throw new EvalFailedException("The Render Settings are still using a skybox for global illumination. Change the Skybox Material render setting.");
    }
